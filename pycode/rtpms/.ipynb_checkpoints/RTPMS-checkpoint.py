...
class RTPMS_OleDB():
    ...
    def __init__(self, rtpms_engine):
        ...
        self.query = "select * from OPENQUERY([SKRTPMS], \'select [time],[tag],[value] from [piarchive]..[piinterp] " \
                     "where [tag] in ({0}) AND timestep = {1} AND [time] BETWEEN {2} AND {3} \')"
        self.time_format = "%Y-%m-%d %H:%M:%S"

    def get_rtpms(self, tag_list, start_time, end_time, time_step, raw_format=False):
    ...
    def get_single_value(self, tag_list, time):
        try:
            ...
            query = "select * from OPENQUERY([SKRTPMS], 'select [tag],[time],[value] from [piarchive]..[piinterp] where tag in ({0}) and time = {1}')".format(
